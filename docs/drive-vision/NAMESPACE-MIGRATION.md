...
分支：`main`。更名前提交：`d52d6c6`。本次只更改專案名稱、Java 套件及相關引用。
...
更名後先比對每個 Java 檔案：扣除套件替換後，內容須與更名前完全相同。另檢查 package 與實際路徑一致、imports 可對應、本地連結存在、原始分析雜湊不變，以及原始版本提交 `79645a4` 未改變（現保存在本機 `codex/original-2025` 分支）。
