...
const graphs = [
    ...
    },
    {
        ...
        nodes: [
            ...
            ['visionio', 'VisionIO inputs\nMT1 + relative target', 'io'],
            ['vision', 'VisionSubsystem\nMT1 first / single-tag gyro fallback\nOne accepted measurement', 'logic'],
    ...
    },
    {
        ...
        nodes: [
            ...
            ['camera', 'limelight-rear / facing FRONT\npriorityid = 21 / tv + tid + target pose', 'input'],
    ...
    {
        id: '05-megatag-inputs', title: '05 / Team 254 MT1 and gyro fallback', direction: 'TB',
        ranks: [['main', 'gyro']],
        nodes: [
            ['camera', 'limelight-rear / front-facing\nCenter + 0.54m / pitch 0', 'input'],
            ['mt1', 'MT1: botpose_wpiblue\nPose + timestamp + fiducials + stddevs', 'io'],
            ['guard', 'Fresh / new timestamp / finite data\nValid tag metadata + positive XY stddevs', 'decision'],
            ['main', '254 MT1 checks\nSingle-tag ambiguity / area / yaw\nOrigin distance / Z / exclusive ID', 'logic'],
            ['gyro', '254 single-tag fallback\nHistorical yaw + known Reef tag\nabs(yaw rate) <= 5 rad/s', 'logic'],
            ['state', 'RobotState\nHistorical pose + angular rate', 'state'],
            ['select', 'Prefer MT1; otherwise gyro fallback\nGyro does not repeat MT1 quality gates', 'decision'],
            ['callback', 'One measurement / one timestamp\nDriveIO: FPGA -> CTRE time ONCE', 'state'],
            ['drive', 'CTRE pose estimator\nVision + gyro + wheel odometry', 'sink'],
            ['relative', 'tv + tid + targetpose_cameraspace\nSquare / Triangle track ID 21\nIndependent of field-pose acceptance', 'logic'],
            ['reject', 'No accepted candidate / useVision=false\nNo field-pose update; log reasons', 'stop'],
        ],
        edges: [
            ['camera', 'mt1', 'Helper read; no MT2'], ['mt1', 'guard', 'raw MT1'],
            ['guard', 'main', 'valid'], ['guard', 'gyro', 'valid'], ['guard', 'reject', 'invalid'],
            ['state', 'main', 'historical yaw'], ['state', 'gyro', 'pose and rate'],
            ['main', 'select', 'MT1 candidate'], ['gyro', 'select', 'fallback candidate'],
            ['select', 'callback', 'selected; useVision=true'], ['select', 'reject', 'none / disabled'],
            ['callback', 'drive', 'latency-aware fusion'], ['drive', 'state', 'updated pose'],
            ['camera', 'relative', 'separate observation'],
        ],
    },
];
