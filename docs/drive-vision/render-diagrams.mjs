...
const graphs = [
    ...
    {
        id: '05-megatag-inputs', title: '05 / MT2 position + MT1 heading', direction: 'TB',
        nodes: [
            ['state', 'RobotState / blue-origin heading\nPigeon angular rate (SIM: wheel rate)', 'state'],
            ['orientation', 'setRobotOrientation BEFORE read\nYaw: degrees / yaw rate: degrees per second\nExternal IMU mode 0', 'logic'],
            ['camera', 'limelight-rear / front-facing\nCenter + 0.54m / pitch 0', 'input'],
            ['mt1', 'MT1: botpose_wpiblue\nHeading + capture timestamp', 'io'],
            ['mt2', 'MT2: botpose_orb_wpiblue\nXY + timestamp + tag count + distance', 'io'],
            ['filter', 'Both valid / fresh / within 50ms\nNo reused frames / field / distance / rotation', 'decision'],
            ['combine', 'One Pose2d: MT2 XY + MT1 heading\nUse MT2 timestamp / heading std = 10 rad', 'logic'],
            ['callback', 'RobotState vision callback\nDriveIO: FPGA -> CTRE time ONCE', 'state'],
            ...
            ['reject', 'No field-pose update\nLog Vision/RejectionReason', 'stop'],
        ],
        edges: [
            ['state', 'orientation', 'every periodic'], ['orientation', 'camera', 'robot_orientation_set'],
            ['camera', 'mt1', 'Helper read'], ['camera', 'mt2', 'Helper read'],
            ['mt1', 'filter', 'required'], ['mt2', 'filter', 'required'],
            ['filter', 'combine', 'accepted'], ['filter', 'reject', 'rejected'],
            ['combine', 'callback', 'one measurement'], ['callback', 'drive', 'latency-aware fusion'],
            ['drive', 'state', 'updated odometry'], ['camera', 'relative', 'separate observation'],
        ],
    },
];
